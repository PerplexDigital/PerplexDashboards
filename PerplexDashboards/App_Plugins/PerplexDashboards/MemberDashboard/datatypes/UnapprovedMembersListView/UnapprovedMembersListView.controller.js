angular.module("umbraco").controller("PerplexDashboards.MemberDashboard.UnapprovedMembersListViewController", [
    "$rootScope",
    "$scope",
    "$routeParams",
    "$injector",
    ...
    with simple values */
        ...
        $scope.approve = function() {
            applySelected(
                function(selected, index) {
                    return perplexMembersDashboardApi.approveMember(getIdCallback(selected[index]));
                },
                function(count, total) {
                    return "Approved " + count + " out of " + total + " member" + (total > 1 ? "s" : "");
                },
                function(total) {
                    return "Approved " + total + " member" + (total > 1 ? "s" : "");
                }
            );
        };
        ...
        $scope.delete = function() {
            applySelected(
                function(selected, index) {
                    return perplexMembersDashboardApi.deleteMember(getIdCallback(selected[index]));
                },
                function(count, total) {
                    return "Deleted " + count + " out of " + total + " member" + (total > 1 ? "s" : "");
                },
                function(total) {
                    return "Deleted " + total + " member" + (total > 1 ? "s" : "");
                }
            );
        };
