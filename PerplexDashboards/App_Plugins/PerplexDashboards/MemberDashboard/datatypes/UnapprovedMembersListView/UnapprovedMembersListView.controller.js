...
.controller("PerplexDashboards.MemberDashboard.UnapprovedMembersListViewController",
...
function listViewController($rootScope, $scope, $routeParams, $injector, $cookieStore, notificationsService, iconHelper, dialogService, editorState, localizationService, $location, appState, $timeout, $q, mediaResource, listViewHelper, perplexMembersDashboardApi) {
    ...
    getListResultsCallback = perplexMembersDashboardApi.getUnapprovedMembersListView;
    ...
    $scope.approve = function () {
        applySelected(
            function (selected, index) { return perplexMembersDashboardApi.approveMember(getIdCallback(selected[index])); },
            function (count, total) { return "Approved " + count + " out of " + total + " member" + (total > 1 ? "s" : ""); },
            function (total) { return "Approved " + total + " member" + (total > 1 ? "s" : ""); });
    }

    $scope.delete = function () {
        applySelected(
            function (selected, index) { return perplexMembersDashboardApi.deleteMember(getIdCallback(selected[index])); },
            function (count, total) { return "Deleted " + count + " out of " + total + " member" + (total > 1 ? "s" : ""); },
            function (total) { return "Deleted " + total + " member" + (total > 1 ? "s" : ""); });
    }
