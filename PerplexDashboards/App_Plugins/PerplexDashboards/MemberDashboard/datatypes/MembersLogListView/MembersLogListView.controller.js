angular.module("umbraco")
    .controller("PerplexDashboards.MemberDashboard.MembersLogListViewController",
['$rootScope', '$scope', '$routeParams', '$injector', '$cookieStore', 'notificationsService', 'iconHelper', 'dialogService', 'editorState', 'localizationService', '$location', 'appState', '$timeout', '$q', 'mediaResource', 'listViewHelper', 'PerplexMembersDashboard.ApiService',
function listViewController($rootScope, $scope, $routeParams, $injector, $cookieStore, notificationsService, iconHelper, dialogService, editorState, localizationService, $location, appState, $timeout, $q, mediaResource, listViewHelper, perplexMembersDashboardApi) {
    ...
    // We willen die Create knop weghalen, dit lijkt the (hacky) way :-)
    // $scope.isNew = true;
    ...
    getListResultsCallback = perplexMembersDashboardApi.getMembersLogListView;
    getIdCallback = function (selected) { return selected.id; };

    // Tegenwoordig hebben members blijkbaar een GUID, maar wij willen een memberId
    // Die moeten we even ophalen via de memberResource
    var getMemberIdDeferred = $q.defer();
    var getMemberIdPromise = getMemberIdDeferred.promise;
    ...
    if (($scope.model.config.entityType && $scope.model.config.entityType === "member") || (appState.getSectionState("currentSection") === "member")) {
        ...
        if ($routeParams.id != null) {
            contentResource.getByKey($routeParams.id).then(function (response) {
                getMemberIdDeferred.resolve(response.id);
            })
        } else {
            getMemberIdDeferred.resolve(null);
        }
    }
    else {
        //check the config for the entity type, or the current section name (since the config is only set in c#, not in pre-vals)
    ...
    $scope.options = {
        ...
        filter: '',
        orderBy: ($scope.model.config.orderBy ? $scope.model.config.orderBy : 'DateTime').trim(),
    ...
    $scope.reloadView = function (id) {
        ...
        getMemberIdPromise.then(function (memberId) {
            getListResultsCallback(memberId, $scope.options).then(function (data) {
                $scope.actionInProgress = false;
    ...
    $scope.unlock = function () {
        applySelected(
            function (selected, index) { return perplexMembersDashboardApi.unlockMember(getIdCallback(selected[index])); },
            function (count, total) { return "Unlocked " + count + " out of " + total + " member" + (total > 1 ? "s" : ""); },
            function (total) { return "Unlocked " + total + " member" + (total > 1 ? "s" : ""); });
    }
